...
- `scan`/`capture --preset NAME` load a built-in policy preset (`default`, `workstation`, `server`, `developer-machine`, `minimal-audit`) directly, for a one-shot run with no separate `policy init`/`--policy` file to manage — mutually exclusive with `--policy`. `default` is a new named alias for `policy.Template`'s existing generic/no-op case (`""`), added to `policy.PresetNames` so it's a discoverable, intentional choice; verified behaviorally identical to omitting `--preset`/`--policy` entirely (root `/`, no `--deep`, auto-safe review — today's existing zero-flag behavior, unchanged).
...
- `recursiveGlob` (`internal/scanner`) now groups `**`-patterns by shared prefix and walks each prefix's base directories once, instead of once per pattern. Several scanners pass a dozen-plus same-prefix patterns in one call (e.g. `scanLanguageProjectHints`'s 19 `/home/*` + 7 `/srv` patterns); on a real host with a large tree under a shared prefix (Go module cache, Rust registry, etc.), this re-walked the same tree dozens of times in a single `scan`. Output is unchanged — same files found, fewer `WalkDir` passes to find them.
- `doctor`'s VM-related subprocess runner (`nix build`, and the `--boot` VM script) now runs in its own process group and is force-killed as a group on timeout, mirroring the identical hardening `internal/scanner/plugin.go` already had. Previously, a `--boot` timeout only killed the VM script's top-level process; if that script doesn't `exec` into `qemu` as its last act, `qemu` would keep running (reparented to init, untracked) and `doctor` would hang waiting on the orphan's inherited output pipe instead of returning at the timeout.
