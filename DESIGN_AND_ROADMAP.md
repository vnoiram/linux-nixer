...
- `baseline fetch` builds a real baseline manifest for a distro release by pulling its official Docker/Podman image and hashing its actual filesystem, so common Ubuntu/Debian/Fedora releases don't require a local rootfs and never rely on hand-curated file data.
...
`internal/baseline/catalog.go` is the curated lookup table from distro/release pairs to verified Docker Hub image references, consumed by `CatalogImage`/`CatalogDigest(distro, release)` and listed via `baseline list`/`CatalogEntries()`. Same conservative-by-construction shape as the Nix mapping table above: an unlisted distro/release stays unlisted rather than being passed through to `docker pull` as a guessed `distro:release` tag, and structural tests in `catalog_test.go` (`TestCatalogKeysAreNormalized`, `TestCatalogDigestsLookLikeSHA256`, `TestCatalogEntriesSorted`) guard the whole table, not just a handful of example lookups. `baseline fetch` rejects any distro/release not in the catalog before attempting a pull, so an unsupported combination fails with a clear message pointing at `baseline list` instead of an opaque `docker pull` error.

Each entry pins the exact image **digest** verified at catalog-authoring time, not just the tag — `Fetch` pulls `image@digest`, never the bare floating tag. This matters specifically because a "baseline" is meant to be a stable, reproducible reference to diff against: a tag like `ubuntu:24.04` gets silently rebuilt over time (routine patches), so pulling by tag alone can't guarantee two fetches of "the same" release — today vs. next month, your machine vs. someone else's — produce identical bytes. This project has no signing infrastructure, so this is content-addressed pinning, not cryptographic signing; refreshing an entry's digest to track a new point release is a deliberate, reviewed catalog change (like updating a mapping entry), never something that happens silently.

Every pull/create is also **fully qualified** to `docker.io/library/<tag>@<digest>` (`qualifiedImageRef` in `catalog.go`), never a bare tag — found to matter in practice, not just in theory: Podman's `registries.conf` short-name aliasing resolved a bare `fedora:40` to `registry.fedoraproject.org/fedora` in one environment, a genuinely different image with a different digest than Docker Hub's own `docker.io/library/fedora:40`. A plain Docker Engine always defaults unqualified names to `docker.io` with no aliasing, so this could silently disagree across environments depending on which container tool and registry configuration is in play. Every catalog entry must be a genuine Docker Hub Official Image (`docker.io/library/*`) for this fully-qualified pull to be correct.

Review checklist for adding a catalog entry:

1. Verify the image reference is a real, current official image on Docker Hub before adding it (e.g. `docker pull docker.io/library/<image>` succeeds, or check hub.docker.com — always pull the fully-qualified `docker.io/library/...` reference explicitly when verifying, not a bare tag, since a bare tag's resolution depends on local registry configuration and may not be what you think it is) — never guess a `distro:release` tag; if it can't be verified, leave the combination out of the catalog rather than add one that might 404 or resolve to something unexpected.
2. After pulling the fully-qualified reference, record its exact digest — e.g. `docker inspect --format='{{index .RepoDigests 0}}' docker.io/library/<image>` (or `docker manifest inspect`) — never guess or hand-derive a digest, and never trust a digest obtained from a bare-tag pull.
3. Add the entry under the correct distro's table with a lowercase, trimmed distro key and a trimmed release key (`CatalogImage`/`CatalogDigest` normalize the distro but not the release, matching how `--release` is typically an exact version string like `24.04`, not free text).
4. Add at least one case to `catalog_test.go` exercising the new entry, then run `go test ./internal/baseline/...` — the structural tests run automatically and need no per-entry maintenance.
5. Where practical, also bundle the entry for fully offline use (see below): run a real `baseline fetch` for it, gzip the resulting manifest into `internal/baseline/baselines_data/<distro>-<release>.json.gz`, and add a lookup case to `embedded_test.go`. Never hand-write a bundled manifest — it must come from a real fetch against the actual image, the same as any other manifest this tool produces.
...
- Review and policy:
  ...
  - `scan`/`capture --preset <name>` (`default`, `workstation`, `server`, `developer-machine`, `minimal-audit`) load a built-in `policy.Template` directly, for a one-shot run with no separate policy file — mutually exclusive with `--policy`. Everything downstream (`scannerOptionsFromFlags`, `reviewOptionsFromFlags`) already consumes a plain `policy.Policy` value, so a `Template`-sourced policy flows through the identical pipeline a `--policy`-loaded one already does; no target-folder auto-detection was added; the existing `--root /` default plus scanners' own `/home/*/...` globbing already covers this. `--preset default` is verified behaviorally identical to omitting `--preset`/`--policy` entirely.
...
- Release:
  - CI runs Go checks.
  - A separate `nix-verify` CI job installs a real Nix toolchain (`DeterminateSystems/determinate-nix-action`), runs `capture` against the CI runner itself to produce a real generated flake, and runs `doctor --vm --boot` against it — a real VM derivation build and a real boot attempt, closing the "designed but never executed against real `nix`" gap for the generated flake/modules. The generated project is migration scaffolding rather than a complete installable host config, so doctor treats known full-system `nix flake check` bootability assertions as non-fatal when the VM derivation build is requested. Standard GitHub-hosted runners genuinely have no `/dev/kvm` (every tier, standard or larger, runs as a nested VM itself — a self-hosted runner on real hardware would be needed to change that, which this project has no infrastructure for), but that turned out not to be a hard blocker for `--boot` specifically: this project never sets `virtualisation.qemu.forceAccel`, so the generated VM script uses nixpkgs' default `-machine accel=kvm:tcg` — QEMU probes for KVM and automatically falls back to software emulation (TCG) rather than failing outright when `/dev/kvm` is absent. An earlier version of this CI job assumed the opposite (that the VM script hard-required KVM) and skipped `--boot` entirely on that premise; it's now attempted with a longer `--timeout` to give a software-emulated boot a real chance, rather than left untested on an unverified assumption. If a TCG boot turns out too slow or flaky in practice, that would be a reason to revisit based on real CI evidence, not the earlier guess.
...
- Audit and guard symlink-following in scanners that stat/read a path derived from scanned content (glob results in `secrets.go`, `devops_config.go`, `backup_config.go`, `user_config.go`, `desktop.go`, and `filesystem.go`'s `WalkDir`): none currently `Lstat` a matched leaf path before `os.Stat`/`os.ReadFile`/`os.Open` on it, so a crafted `--root` filesystem (e.g. a mounted/extracted disk image from a compromised host — exactly this tool's intended use case) containing an absolute symlink at a scanned path could cause the tool to transparently read/hash content from outside the intended root, attributing it to the in-image path in the report. Found during a security-focused audit; not fixed this round because a correct fix needs care across every affected scanner to avoid breaking legitimate in-root symlinks (e.g. `/etc/resolv.conf`, `/bin` → `usr/bin`) — a bigger, more careful change than the other findings from the same audit round.
