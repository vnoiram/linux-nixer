...
- Release:
  - CI runs Go checks.
  - A separate `nix-verify` CI job installs a real Nix toolchain (`DeterminateSystems/determinate-nix-action`), runs `capture` against the CI runner itself to produce a real generated flake, and runs `doctor --vm --boot` against it — a real `nix flake check`, a real VM derivation build, and a real boot attempt, closing the "designed but never executed against real `nix`" gap for the generated flake/modules. Standard GitHub-hosted runners genuinely have no `/dev/kvm` (every tier, standard or larger, runs as a nested VM itself — a self-hosted runner on real hardware would be needed to change that, which this project has no infrastructure for), but that turned out not to be a hard blocker for `--boot` specifically: this project never sets `virtualisation.qemu.forceAccel`, so the generated VM script uses nixpkgs' default `-machine accel=kvm:tcg` — QEMU probes for KVM and automatically falls back to software emulation (TCG) rather than failing outright when `/dev/kvm` is absent. An earlier version of this CI job assumed the opposite (that the VM script hard-required KVM) and skipped `--boot` entirely on that premise; it's now attempted with a longer `--timeout` to give a software-emulated boot a real chance, rather than left untested on an unverified assumption. If a TCG boot turns out too slow or flaky in practice, that would be a reason to revisit based on real CI evidence, not the earlier guess.
