...
func TestProjectWithSingleModuleLayout(t *testing.T) {
	out := t.TempDir()
	report := model.ScanReport{
		SchemaVersion: model.SchemaVersion,
	...
	if err := ProjectWithOptions(out, report, Options{ModuleLayout: "single"}); err != nil {
		t.Fatal(err)
	}
	cfg := readFile(t, out, "hosts/generated/configuration.nix")
	if !strings.Contains(cfg, "../../modules/migration.nix") {
		t.Fatalf("single layout configuration missing migration import:\n%s", cfg)
	}
	for _, unwanted := range []string{"../../modules/services.nix", "../../modules/filesystem-findings.nix", "../../modules/containers.nix"} {
		if strings.Contains(cfg, unwanted) {
			t.Fatalf("single layout configuration included split import %q:\n%s", unwanted, cfg)
		}
	}
	module := readFile(t, out, "modules/migration.nix")
	for _, want := range []string{"# Generated by linux-nixer", "services.cron.systemCronJobs", "/usr/local/bin/tool"} {
		if !strings.Contains(module, want) {
			t.Fatalf("single migration module missing %q:\n%s", want, module)
		}
	}
	for _, rel := range []string{"modules/services.nix", "modules/filesystem-findings.nix", "modules/containers.nix"} {
		if _, err := os.Stat(filepath.Join(out, rel)); !os.IsNotExist(err) {
			t.Fatalf("single layout should not write %s", rel)
		}
	}
}
