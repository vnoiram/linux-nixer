...
func TestProjectRendersOnlyConfirmedPackagesIntoNixSettings(t *testing.T) {
	...
	report := model.ScanReport{
		...
		Packages: []model.Package{
			...
			{Manager: "apt", Name: "shellcheck", NixNames: []string{"shellcheck"}, Decision: model.DecisionCandidate},
	...
	if !strings.Contains(reportMD, "`shellcheck` via apt -> `shellcheck` [candidate]") {
		t.Fatalf("new mapped candidate package missing from report:\n%s", reportMD)
	}
...
func TestProjectRendersConservativeNixOptions(t *testing.T) {
	...
	reportMD := readFile(t, out, "reports/migration-report.md")
	for _, want := range []string{"## Generated Nix summary", "user option: `users.users.alice`", "generated systemd services: `1`", "generated systemd timers: `1`", "generated cron jobs: `1`", "host shell option: `programs.zsh.enable`", "Home Manager option: `programs.git.enable`", "service hint: `systemd.services.custom.enable`"} {
